...
import { getConnectionHandler, newConnectionHandler } from "./controller/connection.controller";
...
io.on("connection", async (socket: Socket) => {
    ...
    //     const connectionQueryString = `INSERT INTO "e-conn-app".private_chats_connections (user_id,connected_user_id) VALUES ($1,$2)`;
    ...
    //     const connectionSearchQueryString = `SELECT "e-conn-app".private_chats_connections.connected_user_id, "e-conn-app".users.username, "e-conn-app".users.email FROM "e-conn-app".private_chats_connections LEFT JOIN "e-conn-app".users ON "e-conn-app".private_chats_connections.connected_user_id = "e-conn-app".users.user_id WHERE "e-conn-app".private_chats_connections.user_id = $1 AND "e-conn-app".private_chats_connections.connected_user_id != "e-conn-app".private_chats_connections.user_id`;
    ...
    //     db.query(updateUserLastSeenQueryString, [user_id]);
    // });
